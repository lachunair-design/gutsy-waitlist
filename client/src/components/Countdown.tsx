...
// Individual digit component to handle the "flip" transition
const FlipDigit = ({ value }: { value: string }) => {
  return (
    <div className="relative h-[1.1em] overflow-hidden leading-none">
      <div 
        key={value}
        className="animate-[slideDown_0.4s_ease-in-out]"
      >
        {value}
      </div>
    </div>
    ...
    <div className="w-full flex justify-between items-center gap-2 md:gap-4 select-none">
      {Object.entries(units).map(([label, value], i) => (
        <div key={label} className="flex flex-col items-center flex-1">
          {/* Numbers: Airport-style Flip Container */}
          <div className="text-[10vw] md:text-[4.5rem] font-black tracking-tightest tabular-nums flex">
            <FlipDigit value={value[0]} />
            <FlipDigit value={value[1]} />
          </div>
          {/* Label: High-authority tracking */}
